### From: https://github.com/SachinKalsi/kmedoids/blob/master/KMedoids.py
...
class KMedoids:
    def __init__(self, n_cluster=2, max_iter=10, tol=0.1, start_prob=0.8, end_prob=0.99, init_strat="kmeans_plusplus_init"):
    ...
    def __initialize_medoids(self): # Updated with the add of the vertices init
        if self.init_strat == "kmeans_plusplus_init":   
        ...
        elif self.init_strat == "vertices_init":   
            '''Vertices initialisation'''
            init_vertices = np.identity(self.n_cluster)
            for k in range(0,self.n_cluster):
                all_l2_vertex_to_data = np.linalg.norm(self.__data  - init_vertices[k].T, axis=1)
                self.medoids.append(np.argmin(all_l2_vertex_to_data))    
        else: 
            print("Please specify kmeans_plusplus_init or vertices_init for medoids initialization:")
